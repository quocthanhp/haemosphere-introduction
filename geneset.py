...
class Geneset(object):
	...
	def subset(self, *args, **kwargs):
		...
		if isinstance(queryStrings, str) or isinstance(queryStrings, six.text_type):	# assume queryString was specified as a string
		...
		# 	for rowIndex,value in six.iteritems(df[column]):
		...
		# 				rowsToKeepTop.add(rowIndex)
		# 				break
		# 			elif matchSubstring and queryString in value:						
		# 				rowsToKeep.add(rowIndex)
		# 				break

		# # Split and recombine the dataframe
		# top_rows_df = df[df.index.isin(rowsToKeepTop)]
		# other_rows_df = df[df.index.isin(rowsToKeep)]
		# combined_df = pandas.concat([top_rows_df, other_rows_df]).drop_duplicates().set_index('EnsemblId')  # Remove duplicates and reset index


		rowsToKeepTop = set()
		rowsToKeepMiddle = set()
		rowsToKeep = set()
		df = df.reset_index()
		for column in searchColumns:
			for rowIndex, value in df[column].items():  # Using items() for iteration in newer Pandas versions
				if rowIndex in rowsToKeepTop or rowIndex in rowsToKeepMiddle or rowIndex in rowsToKeep or not value:
					continue
				...
				for queryString in queryStrings:
					if queryString == value:
						rowsToKeepTop.add(rowIndex)
						break
					elif queryString in value:
						if value.startswith(queryString):
							rowsToKeepMiddle.add(rowIndex)
						else:
							rowsToKeep.add(rowIndex)
						break

		# Split and recombine the dataframe
		top_rows_df = df.loc[rowsToKeepTop]
		middle_rows_df = df.loc[rowsToKeepMiddle]
		other_rows_df = df.loc[rowsToKeep]
		combined_df = pandas.concat([top_rows_df, middle_rows_df, other_rows_df]).drop_duplicates().set_index('EnsemblId')  # Remove duplicates and reset index

		# Save the updated dataframe
		gs._dataframe = combined_df
...
def create_full_geneset(outfile, ensemblFiles, entrezFiles, transcriptLengthFiles, orthologueFile, 
	...
	for fileIndex in range(len(entrezFiles)):
		...
		for entrezId,value in six.iteritems(geneInfoFromEntrezId):
			if value['ensemblId']:
	...
	for ensemblId,_list in six.iteritems(ensemblData):
		listToUse = []
	...
	frameData = []
	for ensemblId,_list in six.iteritems(filteredEnsemblData):
